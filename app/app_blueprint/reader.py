...
def add_to_folder():
    try:
        ...
        # 检查文档是否已在收藏夹中
        existing_content = FolderContent.query.filter_by(
...
def remove_from_folder():
    try:
        ...
        # 查找收藏内容记录
        folder_content = FolderContent.query.filter_by(
            Folder_id=folder.Folder_id,
            Doc_id=data['doc_id']
        ).first()
...
def check_folder(doc_id, folder_name):
    try:
        ...
        # 检查文档是否在收藏夹中
        exists = FolderContent.query.filter_by(
            Folder_id=folder.Folder_id,
            Doc_id=doc_id
        ).first() is not None
